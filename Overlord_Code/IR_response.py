...
def mark_debug_basic(frame, x, y, x_p, y_p, x_c, y_c, t_p, t_ang, x_IR, y_IR):
    out = frame
    if(t_p != None):
...
def offset_corners(offset_corners_x, offset_corners_y, center, offset):
    corners_x = []
    corners_y = []
    tempX = None
    tempY = None
...
#                 x_b_par[i], y_b_par[i] = account_for_parallax((x_b[i], y_b[i]), cameraBorder)
    ...
    x_c, y_c = offset_corners(x_c_offset, y_c_offset, center, CornerOffset)
    ...
    t_p, t_ang = point_from_border_at_dist_and_ang(t_AoA, t_dist, (x_c, y_c))
    ...
    while not done:
        if (DEBUG == True):
            mark_debug_basic(frame, x, y, x_p, y_p, x_c, y_c, t_p, t_ang, x_IR, y_IR)
...
#         print(AoA)
        ...
        if(Collect_ready == True):
            send_to_bot_IR_collect(10)
            Collect_ready = False
            
        if(Waiting_for_update and time.time() >= cam_update_time):
            Waiting_for_update = False
            if(abs(norm_angle(t_ang - angles[0])) > 10 or calc_dist(t_p, (x_IR, y_IR)) > 5):
