...
def extract_centers_new(results, nBots):
    centers = []
    for i in range(nBots):
        temp = []
        temp.append(-1)
...
def mark_bots(results, nBots, frame):
	...
	centers = extract_centers(results, nBots)
	for c in centers:
...
def get_bot_angles(results, nBots):
    ids = extract_ids(results)
    botIndex = []
    i = 0;
    for n in ids:
    ...
    for n in botIndex:
        bots[results[n].tag_id] = results[n]
        bots[n]
        (ptA, ptB, ptC, ptD) = results[n].corners
        ptA = (int(ptA[0]), int(ptA[1]))
        ptB = (int(ptB[0]), int(ptB[1]))
        ptC = (int(ptC[0]), int(ptC[1]))
        ptD = (int(ptD[0]), int(ptD[1]))
        angles[n] = float(degrees(atan2((ptA[0] - ptB[0]), (ptA[1] - ptB[1]))))
    return angles
...
def mark_tags(results, frame):
	for r in results:
		...
		cv2.putText(frame, str(r.tag_id), ptB, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)#(ptB[0], ptB[1] - 20)
	return frame
...
def boundary(results, nBots, frame):
	...
	centers = extract_centers(corners, nBots)
	if (len (centers) == 2):
		(topX, topY) = centers[0]
		(botX, botY) = centers[1]
		topX = int(topX)
